...
# default vnet-cidr is 172.25.16.0/24
...
token=$(az hcs create-federation-token -g $RG --name dc1 | jq -r '.federationToken')
az hcs create --name dc3 --datacenter-name dc3 --resource-group $RG --email hans@hashicorp.com --plan-name on-demand --vnet-cidr 172.25.18.0/24 --federation-token "$token" --external-endpoint enabled > $RG-hcs-dc3.json
